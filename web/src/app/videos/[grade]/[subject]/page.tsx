...
export default async function VideoSubjectPage({
  ...
  return (
    <PlatformLayout active="/videos">
      <Link href={`/videos/${encodeURIComponent(gradeKey)}`} className="text-sm text-kenya-green hover:underline">
        ← {label}
      </Link>
      <h1 className="text-2xl font-bold mt-2">{subject}</h1>
      <p className="text-gray-600">{topics.length} topics — video scripts by topic</p>

      <ol className="mt-8 space-y-3">
        {topics.map((t) => {
          const script = scripts.find(
            (s) => s.topic.subStrand === t.topicName || s.topic.topicNumber === t.topicNumber,
          );
          return (
            <li key={t.topicNumber + t.topicName}>
              {script ? (
                <Link
                  href={`/videos/${encodeURIComponent(gradeKey)}/${encodeURIComponent(subjectSlug)}/${t.slug || t.topicNumber}`}
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-kenya-green">Video script ready</p>
                  </div>
                </Link>
              ) : (
                <div className="flex items-center gap-4 rounded-2xl border border-dashed border-gray-300 p-4 text-gray-400">
                  ...
                  <div>
                    ...
                    <p className="text-xs">Video script coming soon</p>
                  </div>
                </div>
              )}
            </li>
