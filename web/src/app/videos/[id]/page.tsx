      ...
      <article className="mt-4 space-y-6">
        <div className="rounded-2xl bg-kenya-black aspect-video flex items-center justify-center text-white">
          {item.metadata.videoUrl ? (
            <iframe src={item.metadata.videoUrl} className="w-full h-full rounded-2xl" allowFullScreen />
          ) : (
            ...
            <div className="mt-6 space-y-4">
              {item.metadata.scriptSections.map((s, i) => (
                <div key={i} className="rounded-lg border p-4">
                  ...
                  {s.visualCue && <p className="mt-1 text-xs text-gray-400 italic">Visual: {s.visualCue}</p>}
                </div>
              ))}
            </div>
          ) : (
            <div className="mt-6 prose prose-sm whitespace-pre-wrap">{item.body}</div>
          )}
        </div>
      </article>
    </PlatformLayout>
