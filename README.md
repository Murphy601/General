...
**WhatsApp bot** (Node.js, runs in a dry-run/demo mode without real API keys so you can develop
the conversation flow first):
...
Business number via Meta's Cloud API, and [`website/README.md`](website/README.md) for
deployment notes.
...
2. Register as an affiliate on Kilimall, Jumia KOL, AliExpress, and Temu (links + notes in
   `docs/CONCEPT.md`) — approval can take a few days for some programs.
...
4. Replace the demo catalog (`whatsapp-bot/src/data/products.json` and
   `website/data/products.json`) with your first ~20 real curated products and real affiliate
   links.
5. Deploy the bot (Render/Railway/Fly.io/a small VPS) and the website (Netlify/Vercel/Cloudflare
   Pages), point your WhatsApp webhook at the deployed bot URL, and start driving traffic — the
   reels/TikTok style referenced in the original brief works well for this.
