//+------------------------------------------------------------------+
//|                                          EMA_RSI_RiskBot_v4.mq5  |
//|     EMA + RSI + ATR with regime (ADX) & over-extension filters   |
//|                                                                  |
//|  v4 focus: raise entry quality (the real problem in testing).    |
//|   - ADX trend-strength + DI direction filter (skip chop)         |
//|   - Over-extension filter (don't chase price far from EMA)       |
//|   - Exits tuned to let winners reach the 1:2 target              |
//|   - Higher-timeframe trend filter ON                             |
//|   - Margin-aware sizing, verbose logging, init sanity warnings   |
//+------------------------------------------------------------------+
#property strict
...
input group "Regime / Quality Filters"
input bool UseADXFilter = true;            // only trade when a real trend exists (skip chop)
input int ADXPeriod = 14;
input double ADXMinLevel = 22.0;           // require ADX >= this; raise to be more selective
input bool UseADXDirection = true;         // require +DI/-DI to agree with trade direction
input bool UseOverExtensionFilter = true;  // don't chase price too far from the fast EMA
input double MaxDistFromEMA_ATR = 0.8;     // max |close - fastEMA| in ATR units to allow entry
...
input bool UseBreakEven = false;           // OFF by default: early BE was cutting winners short
input double BreakEvenRR = 1.0;            // (used only if UseBreakEven = true)
input int BreakEvenOffsetPoints = 5;
input bool UseTrailingStop = true;
input double TrailStartRR = 1.5;           // start trailing at +1.5R (lets the move develop)
input double TrailATRMult = 2.0;           // wider trail so winners can reach the 1:2 target

input group "Session Filter (Server Time)"
input bool UseSessionFilter = false;       // OFF by default: works for 24h symbols & crypto too
...
void OpenBuy(double atrValue)
{
   ...
   bool ok = trade.Buy(lots, _Symbol, 0.0, sl, tp, "EMA_RSI_v4_BUY");
   if(ok)
   {
      RegisterNewTrade();
...
void OpenSell(double atrValue)
{
   ...
   bool ok = trade.Sell(lots, _Symbol, 0.0, sl, tp, "EMA_RSI_v4_SELL");
   if(ok)
   {
      RegisterNewTrade();
...
void ManageOpenPositions()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      // Break-even (optional; off by default in v4)
...
//+------------------------------------------------------------------+
// Reads ADX (main + DI lines) on the last closed bar.
bool GetADX(double &adxOut, double &plusDIout, double &minusDIout)
{
   if(gAdxHandle == INVALID_HANDLE) return false;
   ...
   if(CopyBuffer(gAdxHandle, 0, 0, 3, adx)   < 3) return false; // MAIN
   if(CopyBuffer(gAdxHandle, 1, 0, 3, plus)  < 3) return false; // +DI
   if(CopyBuffer(gAdxHandle, 2, 0, 3, minus) < 3) return false; // -DI
...
void WarnOnConfig()
{
   ...
   if(PeriodSeconds(gSignalTF) < PeriodSeconds(PERIOD_M15))
      Print("WARNING: timeframe below M15. Spread dominates small targets here; M15/H1 recommended.");
   ...
   {
      if(rr <= 1.05)
         Print("WARNING: reward:risk is ~1:1 (rr=", DoubleToString(rr, 2), "). Use TP >= 2x SL.");
      if(UseBreakEven && BreakEvenRR < rr && BreakEvenRR <= 1.0)
         Print("NOTE: break-even at ", DoubleToString(BreakEvenRR, 2), "R can cut winners before the ",
               DoubleToString(rr, 2), "R target. Disable it if win ratio degrades.");
   }

   if(!UseADXFilter)
      Print("NOTE: ADX regime filter is OFF. Expect more trades in chop and a lower win rate.");
}
...
int OnInit()
{
   if(FastEMA <= 0 || SlowEMA <= 0 || RSIPeriod <= 0 || ATRPeriod <= 0 || TrendEMAPeriod <= 0 || ADXPeriod <= 0)
...
void OnTick()
{
   ...
   // ADX regime filter: only trade when a real trend exists, with DI direction.
   bool adxBull = true;
   bool adxBear = true;
   if(UseADXFilter)
   {
      ...
      bool adxStrong = (adxVal >= ADXMinLevel);
      bool diBull = (!UseADXDirection) || (plusDI > minusDI);
      bool diBear = (!UseADXDirection) || (minusDI > plusDI);

      adxBull = adxStrong && diBull;
      adxBear = adxStrong && diBear;

      if(!adxStrong)
         LogSkipOncePerBar(StringFormat("ADX %.1f < %.1f (no trend / chop)", adxVal, ADXMinLevel));
   }

   // Over-extension filter: don't chase price too far from the fast EMA.
   bool extensionOK = true;
   if(UseOverExtensionFilter)
   {
      double close1 = iClose(_Symbol, gSignalTF, 1);
      if(close1 > 0.0)
      {
         double distATR = MathAbs(close1 - fast[1]) / atr[1];
         extensionOK = (distATR <= MaxDistFromEMA_ATR);
      }
   }
   ...
   {
      bool immBuy  = emaBull && (!ImmediateRequireRSI || rsiBull) && trendBull && adxBull && extensionOK;
      bool immSell = emaBear && (!ImmediateRequireRSI || rsiBear) && trendBear && adxBear && extensionOK;
   ...
   bool buySignal = bullishSetup && rsiBull && trendBull && adxBull && extensionOK;
   bool sellSignal = bearishSetup && rsiBear && trendBear && adxBear && extensionOK;
