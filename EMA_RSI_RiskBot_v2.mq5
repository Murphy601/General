...
input bool UseStateEntry = true;           // enter on CURRENT EMA alignment (no need to wait for a fresh crossover)
input bool TradeImmediatelyOnStart = true; // fire on the first allowed tick after attaching / enabling algo trading
...
// Set true in OnInit when TradeImmediatelyOnStart is enabled. Fires once on the
// first allowed tick so the EA trades right after algo trading is permitted.
...
int OnInit()
{
   ...
   // Arm the one-shot immediate entry so the EA trades on the first allowed tick
...
void OnTick()
{
   ...
   // Current EMA alignment (state) on the last closed bar.
   bool emaBull = (fast[1] > slow[1]);
   bool emaBear = (fast[1] < slow[1]);

   // ----------------------------------------------------------------
   // Immediate start entry (one-shot):
   // Right after algo trading is enabled, open a trade based on the CURRENT
   // EMA alignment without waiting for a fresh crossover. Runs every tick until
   // it can fire once, then normal logic takes over.
   // ----------------------------------------------------------------
   if(gImmediateEntryPending && !hasOpenPositionNow)
   {
      ...
      if((immBuy || immSell) && CooldownPassed() && gTradesToday < MaxTradesPerDay)
      {
         ...
         return; // immediate entry handled for this tick
      }
   }

   // Normal signal evaluation runs once per new signal bar.
   if(!IsNewSignalBar()) return;

   // Entry direction:
   //  - UseStateEntry = true  -> use the current EMA alignment (no fresh crossover required).
   //  - UseStateEntry = false -> require a fresh EMA crossover within the lookback window.
